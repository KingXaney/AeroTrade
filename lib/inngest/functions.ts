...
export const sendDailyNewsSummary = inngest.createFunction(
    ...
    async ({ step }) => {
        ...
        // Step #2: For each user, get watchlist symbols and fetch relevant news
        ...
        await step.run('send-news-emails', async () => {
            await Promise.all(
                userNewsSummaries.map(async({user, newsContent}) => {
                    ...
                    return await sendNewsSummaryEmail({email: user.email, date: formatDateToday, newsContent: newsContent})
                })
            )
        })
